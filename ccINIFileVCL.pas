...
{=======================================================================================================================
  ...
  Features:
     ...
     * Allows you to store/restore the whole GUI state (checkboxes, radiobuttons, etc) with just one function call.
  ____________
  ...
  Compleateness:
     ...
     Use IsSupported() to see if this class can store the specified control.
     The next item to be supported: TCheckListBox
  ...
  Utils:
     Use ccIniFile.AppIniFile to obtain the file name of the INI file (where to save the data).
     Use ccIniFile.AppDataFolder to get the path where the INI file is stored.
...
{ These only support standard VCL controls. If you want to save also the custom (Cubic) controls see cvIniFileVclEx }
 ...
 then
  ...
  then
   begin
    ...
    if (s > '') then WriteString (Comp.Owner.Name, Comp.Name, s);
   end else
  ...
  if ValueExists(Comp.Owner.Name, Comp.Name) then
   begin
      ...
      if Comp.InheritsFrom(TOpenDialog) then
       begin
         s:= ReadString (Comp.Owner.Name, Comp.Name, '');  { NOTE: INI file does not support unicode chars }
